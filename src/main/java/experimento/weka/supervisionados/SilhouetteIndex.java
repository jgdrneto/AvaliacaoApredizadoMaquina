...
public class SilhouetteIndex implements Serializable{
	...
	public void evaluate(AbstractClusterer clusterer,List<Instance> centroids, 
		...
		for (int i = 0; i < clusteredInstances.length; i++) {
			...
			for (int j = 0; j < clusteredInstances[i].size(); j++) {
				...
				for (int k = 0; k < clusteredInstances[i].size(); k++) {
					...
					meanDistSameC += MLClusterer.distance(i1, i2);
				}
				...
				for (int k = 0; k < centroids.size(); k++) {
					...
					double distance = MLClusterer.distance(i1, i2);
				...
				for (int k = 0; k < clusteredInstances[minCentroid].size(); k++) {
					...
					/* Distance. */
					meanDistOtherC += MLClusterer.distance(i1, i2);
				}
			...
			if(Double.isNaN(centroidSilhouetteIndex)) {
